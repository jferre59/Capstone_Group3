...
class Preprocessor:
    ...
    def prepare_for_model(self, df):
        ...
        # REFACTORED: Explicitly remove leakage proxies (id/code/target-like fields) to fix 99% leakage bug.
        leakage_keywords = ('treatment', 'target', 'label', 'class', 'id', 'code')
        feature_cols = [
        ...
        #Defing X and y
        X = df[feature_cols]
        df['treatment'] = df['treatment'].fillna(df['treatment'].mode()[0]) #Fill missing values with mode of column
        y = df['treatment'] 
...
if __name__ == "__main__":
    ...
    output_model = os.path.join(backend_dir, 'model', 'trained_model_v2_treat.joblib')
    os.makedirs(os.path.dirname(output_model), exist_ok=True)
